...
let inp = document.querySelector(".task-input1");
let inp1 = document.querySelector(".task-input2");
let inp2 = document.querySelector(".task-input3");
...
btn.addEventListener("click", () => {
    ...
    console.log(task)
    // создаём новый объект с ключём task и со значением инпута (inp)
    setItemToStorage(task); // вызываем функцию, которая добавляет наш новый созданный объект в хранилище localStorage c ключом task-data
    render();// для
    // того чтобы, все данные отобразились сразу после нажатия на кнопку Add task
    inputs.value = ""; // очищает инпут
});
...
function render() {
    ...
    newData.forEach((item, index) => {
        ...
        li3.style.height = "300px"
        li3.style.border="1px solid"
...
function editElement(id) {
    ...
    let data = JSON.parse(localStorage.getItem("task-data"));
    inpEdit.setAttribute("id"); // для того чтобы сохранить нужный нам индекс в аттрибут инпута в виде id
